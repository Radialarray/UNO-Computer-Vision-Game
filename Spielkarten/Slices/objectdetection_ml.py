...
for node in listdir(trainingDataFolder):
    ...
    if(isdir(join(trainingDataFolder,node))):
        ...
        for f in listdir(join(trainingDataFolder, node)):
            if isfile(join(trainingDataFolder, node, f)) and f != ".DS_Store" and not f.startswith(('icon', 'Icon', 'Icon?')):
...
image_uno = cv2.imread("fuenf.jpg", 0)

image_uno_gray = cv2.resize(image_uno, (dimensions, dimensions), interpolation = cv2.INTER_AREA)

features_uno = hog.compute(image_uno_gray).T

print("Classification results:")
print("=======================")
print("Prediction for uno image: "+str(clf.predict(features_uno)))
